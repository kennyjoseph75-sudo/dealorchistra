...
function DealInfoTab({ deal, state, dispatch, readOnly }) {
  ...
  const archive = async () => {
    ...
    try {
      await sb.update("deals", deal.id, { archived:true, state:"closed_won" });
      dispatch({ type:"UPDATE_DEAL", id:deal.id, updates:{ archived:true, state:"closed_won" } });
    } catch (e) { console.error(e); }
  };
